...
(function (window, document, $) {
  ...
  $(window).on("load", function () {
    ...
    // get menu
    $.getJSON("auth/menu/search/json", function (data) {
      $datasearch = data;
    });
  ...
  $(".bookmark-wrapper .bookmark-star").on("click", function (e) {
    ...
    for (var i = 0; i < arrList.length; i++) {
      ...
      var $icons = arrList[i].firstChild.dataset.icon;
      var icon_arr = $icons.split(",");
      var $menu_id = icon_arr[1];
      var $url_menu = icon_arr[2];

      var $names = arrList[i].firstChild.dataset.originalTitle;
      var names_arr = $names.split("^");
      var $menu_name = names_arr[0].trim();
      ...
      $arrList +=
        ...
        '<a class="d-flex align-items-center justify-content-between w-100" href="javascript:void(0)" onclick="main_menu.openMenuTabs(' + $menu_id + ', \'' + $menu_name + '\', \'' + $url_menu + '\')" data-icon="' + arrList[i].firstChild.dataset.icon + '">' +

        '<div class="d-flex justify-content-start">' +
        '<span class="mr-75 ' +
        arrList[i].firstChild.firstChild.className +
        '"  data-icon="' +
        arrList[i].firstChild.dataset.icon +
        '"></span>' +
        "<span>" +
  ...
  $(".search-input .input").on("keyup", function (e) {
    ...
    if (e.keyCode !== 38 && e.keyCode !== 40 && e.keyCode !== 13) {
      ...
      if (value != "") {
        ...
        if (jQuery.isEmptyObject($datasearch)) {
          $.getJSON("auth/menu/search/json", function (data) {
            $datasearch = data;
            ...
            for (var i = 0; i < data.listItems.length; i++) {
                ...
                if (
                    ...
                    $startList +=
                        ...
                        'onclick = "main_menu.openMenuTabs(' + data.listItems[i].id + ', \'' + data.listItems[i].menu + '\', \'' + data.listItems[i].url + '\')"' +
                        '>' +
                        '<div class="d-flex justify-content-start">' +
                        '<span class="mr-75 ' +
                        data.listItems[i].icon +
                        '" data-icon="' +
                        data.listItems[i].icon + ',' + data.listItems[i].id + ',' + data.listItems[i].url +
                        '"></span>' +
                        "<span>" +
                        data.listItems[i].name +
                        "</span>" +
            ...
            if ($startList == "" && $otherList == "") {
                $otherList =
                    ...
                    "<span>" + global.message.error.not_found + "</span>" +
                    "</div>" +
                    "</a>" +
                    "</li>"
            }
  ...
  $(document).on(
    ...
    function (e) {
      ...
      var $url = $(this).parent()[0].href,
          $names = $(this).parent()[0].innerText,
          $icons = $(this).parent()[0].firstChild.firstChild.dataset.icon
      ...
      if ($(this).hasClass("warning")) {
        ...
        $listItem =
          ...
          '<a class="nav-link" href="javascript:void(0)" onclick="main_menu.openMenuTabs(' + $menu_id + ', \'' + $menu_name + '\', \'' + $url_menu + '\')" data-toggle="tooltip" data-placement="top" title="' +
          $names +
          '" data-icon="' + $icons + '">' +
          '<i class="ficon ' +
          $icon +
  ...
  $(window).on("keydown", function (e) {
    ...
    if (e.keyCode === 13 && $(".search-list li.current_item").length > 0) {
      ...
      $(".search-list li.current_item").removeClass("current_item")
