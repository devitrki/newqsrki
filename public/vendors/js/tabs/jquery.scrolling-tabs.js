...
;(function ($, window) {
  ...
  var tabElements = (function () {
    ...
    function getNewElTabAnchor(tab, propNames) {
      return $('<a class="nav-link" role="tab" data-toggle="tab"><span class="align-middle">' + tab[propNames.title] + '</span></a>')
              .attr('href', '#' + tab[propNames.paneId]);
    }

    function getNewElTabCloseAnchor(tab, propNames) {
      return $('<a href="javascript:void(0)" onclick="main_menu.closeTab(\'' + tab.paneId + '\')" class="tabs-close"><i class="bx bx-x"></i></a>');
    }

    function getNewElTabContent() {
      return $('<div class="tab-content" style="height: ' + (global.display.content_wrapper_height - 32) + 'px;"></div>');
    }

    function getNewElTabLi(tab, propNames, options) {
      var liContent = options.tabLiContent || '<li role="presentation" class="nav-item"></li>',
          $li = $(liContent),
          $a = getNewElTabAnchor(tab, propNames).appendTo($li),
          $close = getNewElTabCloseAnchor(tab, propNames).appendTo($li);
    ...
    function getNewElTabPane(tab, propNames, options) {
      $('.tab-content').block({
          ...
          message: '<div class="semibold"><span class="bx bx-revision icon-spin text-left"></span>&nbsp; ' + global.label.loading + ' ...</div>',
          overlayCSS: {
              backgroundColor: "#fff",
              opacity: 0.8,
              cursor: 'wait'
      ...
      var $pane = $('<div role="tabpanel" class="tab-pane"></div>')
                  ...
                  .load(tab[propNames.content] , function (response, status, xhr) {
                      main_menu.myNewTabOpen = [];

                      $('.tab-content').unblock();
                      if (status == "success") {
                          // inisialisasi
                          $(".fit-screen-tabs").height(global.display.content_wrapper_height - 32);
                          ...
                          $('.dropdown-menu.dropdown-menu-filter').on('click', function (event) {

                              event.stopPropagation();

                          });

                          var content_tabs = $('.content-scroll');
                          if (content_tabs.length > 0) {
                            new PerfectScrollbar(".content-scroll", {
                                wheelPropagation: false
                            });
                          }
                      } else {
                          $(this).load('error', function (resp, stat, x) {
                            if (stat == "success") {
                              $(".fit-screen-tabs").height(global.display.content_wrapper_height - 32);
                              global.display.content_tabs_height = global.display.content_wrapper_height - 74;
                              $('.fit-content-tabs').height(global.display.content_tabs_height);
