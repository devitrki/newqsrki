  ...
  function () {
    ...
    _createClass(HeatMap, [{
      ...
      value: function determineHeatColor(i, j) {
        ...
        var percent = 100 * val / (total === 0 ? total - 001 : total);
        ...
        if (heatmap.colorScale.ranges.length > 0) {
          ...
          colorRange.map(function (range, index) {
            if (val >= range.from && val <= range.to) {
              ...
              percent = 100 * val / (_total === 0 ? _total - 001 : _total);
            }
          });
        }
        ...
        for (var i = 0; i < series.length; i++) {
          ...
          if (type === 'line' && (w.config.fill.type === 'gradient' || w.config.fill.type[i] === 'gradient')) {
            ...
            if (coreUtils.seriesHaveSameValues(i)) {
              ...
              gSeries[gSeries.length - 1] = gSeries[gSeries.length - 1] + 001;
              series[i] = gSeries;
            }
          }
    ...
    _createClass(Utils, [{
      ...
      value: function getNearestValues(_ref) {
        ...
        for (var s = 0; s < w.globals.seriesXvalues.length; s++) {
          seriesXValArr.push([w.globals.seriesXvalues[s][0] - 001].concat(w.globals.seriesXvalues[s]));
        }
