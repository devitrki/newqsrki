<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="inventory/usedoil/uo-good-receipt/dtble?plant-id={{ $first_plant_id }}&from={{ date('Y/m/d', strtotime('-30 days')) }}&until={{ date('Y/m/d') }}" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Good Receipt" size="lg">
    <x-form-horizontal>
...
<x-modal :dom="$dom" compid="modalcancel" title="Cancel Good Receipt">
    <x-form-horizontal>
        <x-row-horizontal label="PIC">
...
<x-modal :dom="$dom" compid="modalview" title="View Good Receipt" size="lg">
    <x-form-horizontal>
        <x-row-horizontal label="Plant">
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "inventory/usedoil/uo-good-receipt",
        datatable: "inventory/usedoil/uo-good-receipt/dtble",
        datatableView: "inventory/usedoil/uo-good-receipt/dtble/view/",
        urlDtble: "inventory/usedoil/uo-material/dtble/qty/uogoodreceipt/"
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
    ...
    func: {
        ...
        getDataForm: function () {
            ...
            return {
                ...
                'qty': $("input[name='uogoodreceipt[]']").map(function(){return $(this).val();}).get(),
                'id': {{$dom}}.data.id
            }
        },
        save: function () {
