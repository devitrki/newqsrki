<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="logbook/operational/env-wastewater/dtble?plant-id={{$first_plant_id}}&month={{$months[ date('n') - 1 ]['id']}}&year={{date('Y')}}" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Environmental Wastewater">
    <x-form-horizontal>
        ...
        <x-row-horizontal label="volume inlet">
            <input type="text" class="form-control form-control-sm" id="volume_inlet{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="volume outlet">
            <input type="text" class="form-control form-control-sm" id="volume_outlet{{$dom}}">
        </x-row-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "logbook/operational/env-wastewater",
        datatable: "logbook/operational/env-wastewater/dtble"
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
    ...
    func: {
        ...
        reset: function () {
            ...
            $("#volume_inlet{{$dom}}").val('');
            $("#volume_outlet{{$dom}}").val('');
            $("#pic{{$dom}}").val('');
        ...
        set: function () {
            ...
            $("#volume_inlet{{$dom}}").val(data.volume_inlet);
            $("#volume_outlet{{$dom}}").val(data.volume_outlet);
        ...
        getDataForm: function () {
            return {
                ...
                'volume_inlet': $("#volume_inlet{{$dom}}").val(),
                'volume_outlet': $("#volume_outlet{{$dom}}").val(),
                'pic': $("#pic{{$dom}}").val(),
                'id': {{$dom}}.data.id
            }
        },
