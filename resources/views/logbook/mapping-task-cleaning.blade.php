<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="logbook/mapping/task-cleaning/dtble" :select="[true, 'single']"/>
        </div>
    </div>
</x-card-scroll>

<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Task Cleaning and Sanitation">
    <x-form-horizontal>
        <x-row-horizontal label="Task">
            <input type="text" class="form-control form-control-sm" id="task{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Section">
            <select class="form-control form-control-sm" id="section{{$dom}}">
                @php
                    $options = [
                                ['value' => 'Lobby', 'text' => 'Lobby'],
                                ['value' => 'Counter', 'text' => 'Counter'],
                                ['value' => 'Drive Thru', 'text' => 'Drive Thru']
                            ];
                @endphp
...
{{$dom}} = {
    ...
    url: {
        save: "logbook/mapping/task-cleaning",
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
