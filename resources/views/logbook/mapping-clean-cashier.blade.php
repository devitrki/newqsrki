<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="logbook/mapping/task-clean-cashier/dtble" :select="[true, 'single']"/>
        </div>
    </div>
</x-card-scroll>

<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Task Cleaning Duties Cashier">
    <x-form-horizontal>
        <x-row-horizontal label="Task">
            <input type="text" class="form-control form-control-sm" id="task{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Frekuensi">
            <select class="form-control form-control-sm" id="frekuensi{{$dom}}">
                @php
                    $options = [
        ...
        <x-row-horizontal label="Day">
            <select class="form-control form-control-sm" id="day{{$dom}}">
                @php
                    $options = [
                                ['value' => 'Monday', 'text' => \Lang::get('Monday')],
                                ['value' => 'Tuesday', 'text' => \Lang::get('Tuesday')],
                                ['value' => 'Wednesday', 'text' => \Lang::get('Wednesday')],
                                ['value' => 'Thursday', 'text' => \Lang::get('Thursday')],
                                ['value' => 'Friday', 'text' => \Lang::get('Friday')],
                                ['value' => 'Saturday', 'text' => \Lang::get('Saturday')],
                                ['value' => 'Sunday', 'text' => \Lang::get('Sunday')]
                            ];
                @endphp
                @foreach( $options as $opt)
                    <option value="{{$opt['value']}}">{{$opt['text']}}</option>
...
{{$dom}} = {
    ...
    url: {
        save: "logbook/mapping/task-clean-cashier",
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
    ...
    func: {
        ...
        set: function () {
            ...
            data = row_data[0];
            {{$dom}}.data.id = data.id;
            $("#task{{$dom}}").val(data.task);
            $("#section{{$dom}}").val(data.section);
