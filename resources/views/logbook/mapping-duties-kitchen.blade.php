<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="logbook/mapping/task-duties-kitchen/dtble" :select="[true, 'single']"/>
        </div>
    </div>
</x-card-scroll>

<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Task Duties Kitchen">
    <x-form-horizontal>
...
{{$dom}} = {
    ...
    url: {
        save: "logbook/mapping/task-duties-kitchen",
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
