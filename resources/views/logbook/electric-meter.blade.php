<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="logbook/operational/electric-meter/dtble?plant-id={{$first_plant_id}}&month={{$months[ date('n') - 1 ]['id']}}&year={{date('Y')}}" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Electric Meter">
    <x-form-horizontal>
        ...
        <x-row-horizontal label="Initial Meter">
            <input type="text" class="form-control form-control-sm" id="initial_meter{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Final Meter">
            <input type="text" class="form-control form-control-sm" id="final_meter{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Usage">
            <input type="text" class="form-control form-control-sm" id="usage{{$dom}}" readonly>
        </x-row-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "logbook/operational/electric-meter",
        datatable: "logbook/operational/electric-meter/dtble"
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
