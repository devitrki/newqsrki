<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="logbook/operational/env-propump/dtble?plant-id={{$first_plant_id}}&month={{$months[ date('n') - 1 ]['id']}}&year={{date('Y')}}" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Environmental Propump">
    <x-form-horizontal>
        ...
        <x-row-horizontal label="DGTT SA">
            <input type="text" class="form-control form-control-sm" id="dgtt_sa{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="HC">
            <input type="text" class="form-control form-control-sm" id="hc{{$dom}}">
        </x-row-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "logbook/operational/env-propump",
        datatable: "logbook/operational/env-propump/dtble"
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
