<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="logbook/mapping/payment-list/dtble" :select="[true, 'single']"/>
        </div>
    </div>
</x-card-scroll>

<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Payment List">
    <x-form-horizontal>
        <x-row-horizontal label="Payment">
            <input type="text" class="form-control form-control-sm" id="payment{{$dom}}">
        </x-row-horizontal>
...
{{$dom}} = {
    ...
    url: {
        save: "logbook/mapping/payment-list",
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
