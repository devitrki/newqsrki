<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="logbook/operational/gas-meter/dtble?plant-id={{$first_plant_id}}&month={{$months[ date('n') - 1 ]['id']}}&year={{date('Y')}}" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "logbook/operational/gas-meter",
        datatable: "logbook/operational/gas-meter/dtble"
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
