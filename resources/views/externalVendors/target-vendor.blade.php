            ...
            @php
                $columns =
                    [
                        ...
                        [
                            'label' => 'transfer type',
                            'data' => 'transfer_type_desc',
                            'searchable' => 'true',
                            'orderable' => 'false',
                        ],
                    ];
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="external-vendor/target-vendor/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Target Vendor">
    <x-form-horizontal>
        ...
        <x-row-horizontal label="Transfer Type">
            <x-select :dom="$dom" compid="transfer_type" type="array" :options="$transfer_type_options" size="sm"/>
        </x-row-horizontal>
    </x-form-horizontal>
...
<x-modal :dom="$dom" compid="modalconf" title="Target Vendor Configuration" size="lg">
    <x-form-horizontal>
        <div class="row">
            <div class="col-12">
...
<x-modal :dom="$dom" compid="modalmanageconf" title="Target Vendor Configuration" close="false">
    <x-form-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "external-vendor/target-vendor",
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
    ...
    func: {
        ...
        set: function () {
            ...
            fslcttransfer_type{{$dom}}.set(data.transfer_type, data.transfer_type_desc);
            showModal('modalmanage{{$dom}}');
        },
        getDataForm: function () {
            return {
                'name': $("#name{{$dom}}").val(),
                'transfer_type': fslcttransfer_type{{$dom}}.get(),
                'id': {{$dom}}.data.id
            }
        },
        save: function () {
        ...
        initConf: function () {
            ...
            $("#modalconf{{$dom}}-title").html("Company " + data.name);
