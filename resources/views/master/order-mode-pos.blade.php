<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="master/order-mode-pos/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Uom Convert">
    <x-form-horizontal>
        <x-row-horizontal label="Order Mode ID">
            <input type="text" class="form-control form-control-sm" id="order_mode_id{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Order Mode Name">
            <input type="text" class="form-control form-control-sm" id="order_mode_name{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="SAP Name">
            <input type="text" class="form-control form-control-sm" id="sap_name{{$dom}}">
        </x-row-horizontal>
    </x-form-horizontal>
   ...
   data: {
    ...
    func: {
        ...
        set: function () {
            ...
            $("#order_mode_id{{$dom}}").val(data.order_mode_id);
            $("#order_mode_name{{$dom}}").val(data.order_mode_name);
            $("#sap_name{{$dom}}").val(data.sap_name);

            showModal('modalmanage{{$dom}}');
        },
        getDataForm: function () {
            return {
                'order_mode_id': $("#order_mode_id{{$dom}}").val(),
                'order_mode_name': $("#order_mode_name{{$dom}}").val(),
                'sap_name': $("#sap_name{{$dom}}").val(),
                'id': {{$dom}}.data.id
            }
        },
        save: function () {
