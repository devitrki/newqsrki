<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="master/uom-convert/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<x-modal :dom="$dom" compid="modalmanage" title="Uom Convert">
    <x-form-horizontal>
        <x-row-horizontal label="Base Uom">
            <input type="text" class="form-control form-control-sm" id="base_uom{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Send SAP Uom">
            <input type="text" class="form-control form-control-sm" id="send_sap_uom{{$dom}}">
        </x-row-horizontal>
    </x-form-horizontal>
   ...
   data: {
    ...
    func: {
        ...
        set: function () {
            ...
            $("#base_uom{{$dom}}").val(data.base_uom);
            $("#send_sap_uom{{$dom}}").val(data.send_sap_uom);

            showModal('modalmanage{{$dom}}');
        },
        getDataForm: function () {
            return {
                'base_uom': $("#base_uom{{$dom}}").val(),
                'send_sap_uom': $("#send_sap_uom{{$dom}}").val(),
                'id': {{$dom}}.data.id
            }
        },
        save: function () {
