<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="master/pettycash-glcc/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Master Petty Cash GL CC">
    <x-form-horizontal>
        <x-row-horizontal label="GL">
            <input type="text" class="form-control form-control-sm" id="gl{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="CC">
            <input type="text" class="form-control form-control-sm" id="cc{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Privilege">
            @php
                $options = [
                            ['id' => 0, 'text' => __('All')],
                            ['id' => 1, 'text' => 'Outlet'],
            ...
            @endphp
            <x-select :dom="$dom" compid="privilege" type="array" :options="$options" size="sm"/>
        </x-row-horizontal>
    </x-form-horizontal>
   ...
   data: {
    ...
    func: {
        ...
        set: function () {
            ...
            $("#gl{{$dom}}").val(data.gl);
            $("#cc{{$dom}}").val(data.cc);
            fslctprivilege{{$dom}}.set(data.privilege);
            showModal('modalmanage{{$dom}}');
        },
        getDataForm: function () {
            return {
                'gl': $("#gl{{$dom}}").val(),
                'cc': $("#cc{{$dom}}").val(),
                'privilege': fslctprivilege{{$dom}}.get(),
                'id': {{$dom}}.data.id
            }
        },
        save: function () {
