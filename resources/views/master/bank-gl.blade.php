<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="master/bank-gl/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
   ...
   data: {
    ...
    func: {
        ...
        set: function () {
            ...
            $("#bank_gl{{$dom}}").val(data.bank_gl);
            $("#bank{{$dom}}").val(data.bank);

            $("#bank{{$dom}}").prop("disabled", true);
            showModal('modalmanage{{$dom}}');
        },
        getDataForm: function () {
            return {
                'bank_gl': $("#bank_gl{{$dom}}").val(),
                'bank': $("#bank{{$dom}}").val(),
                'id': {{$dom}}.data.id
            }
        },
