<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="master/language/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Language">
    <x-form-horizontal>
        <x-row-horizontal label="Language">
            <input type="text" class="form-control form-control-sm" id="lang{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Short Language">
            <input type="text" class="form-control form-control-sm" id="short_lang{{$dom}}">
        </x-row-horizontal>
    </x-form-horizontal>
   ...
   data: {
    ...
    func: {
        ...
        set: function () {
            ...
            $("#lang{{$dom}}").val(data.lang);
            $("#short_lang{{$dom}}").val(data.short_lang);
            showModal('modalmanage{{$dom}}');
        },
        getDataForm: function () {
            return {
                'lang': $("#lang{{$dom}}").val(),
                'short_lang': $("#short_lang{{$dom}}").val(),
                'id': {{$dom}}.data.id
            }
        },
        save: function () {
