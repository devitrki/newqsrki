    ...
    <body>
        <div class="wrapper">
            ...
            <table class="sp-header"  style="width: 100%; margin: 0 auto;">
                ...
                <tr>
                    <td rowspan="4" valign="top" width="30%">
                        {{ $header['plant_sender_code'] . ' - ' . $header['plant_sender'] }}
                        <br/>
                        {{ $header['plant_sender_address'] }}
                    </td>
                    <td>{{ App\Library\Helper::DateConvertFormat($header['date_confirmation_sender'], 'Y-m-d H:i:s', 'd/m/Y') }}</td>
                    <td rowspan="4" valign="top">
                        {{ $header['plant_receiver_code'] . ' - ' . $header['plant_receiver'] }}
                        <br/>
                        {{ $header['plant_receiver_address'] }}
                    </td>
                </tr>
                <tr>
                    <td></td>
            ...
            <table class="sp-item" style="width: 100%; margin: 0 auto; margin-top: 20px;" cellpadding="0" cellspacing="0">
                ...
                <tr class="item">
                    <td>{{ $row['number'] }}</td>
                    <td>{{ $row['number_sub'] }}</td>
                    <td>{{ $row['description'] }}</td>
                    <td>{{ $row['spec_user'] }}</td>
                    <td align="right">{{ $row['qty_mutation'] }}</td>
                    <td>{{ $row['uom'] }}</td>
                    <td>{{ $row['remark'] }}</td>
                </tr>
                @endforeach
