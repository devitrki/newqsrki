...
<x-modal :dom="$dom" compid="modalexport" title="Export Report">
    <x-form-horizontal>
        ...
        <x-row-horizontal label="Year">
            <x-select :dom="$dom" compid="eyear" type="array" :options="$years" size="sm" :default="[date('Y'), date('Y')]"/>
        </x-row-horizontal>
        <x-row-horizontal label="Month">
            <x-select :dom="$dom" compid="emonth" type="array" :options="$months" size="sm" :default="[$months[ date('n') - 1 ]['id'], $months[ date('n') - 1  ]['text']]"/>
        </x-row-horizontal>
        <x-row-horizontal label="Material">
            <x-select :dom="$dom" compid="ematerial" type="serverside" url="master/material-logbook/select?limit=10" size="sm" autocomplete="true"/>
        </x-row-horizontal>
    </x-form-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        report: "report/logbook/stock-card/report",
        export: "report/logbook/stock-card/export",
    },
    event: {
        excel: function () {
    ...
    func: {
        show: function () {
            ...
            var url ={{$dom}}.url.report + '?plant-id=' + fslctfplant{{$dom}}.get() +
                        '&month=' + fslctfmonth{{$dom}}.get() +
                        '&year=' + fslctfyear{{$dom}}.get() +
                        '&material=' + fslctfmaterial{{$dom}}.get();
        ...
        getDataForm: function () {
            return {
                ...
                'month': fslctemonth{{$dom}}.get(),
                'year': fslcteyear{{$dom}}.get(),
                'material': fslctematerial{{$dom}}.get(),
