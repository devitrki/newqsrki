                        ...
                        <x-dropdown-filter :dom="$dom" dtblecompid="tabledata">
                            <div class="col-12">
                                <x-form-vertical>
                                    ...
                                    <x-row-vertical label="Storage">
                                        <x-select :dom="$dom" compid="fstorage" type="serverside" url="logbook/mapping/storage-temp/select" size="sm" dropdowncompid="tabledata" :default="[$first_storage_name, $first_storage_name]"/>
                                    </x-row-vertical>
                                </x-form-vertical>
                            </div>
...
<x-modal :dom="$dom" compid="modalexport" title="Export Report">
    <x-form-horizontal>
        ...
        <x-row-horizontal label="From Date">
            <x-pickerdate :dom="$dom" compid="efromdate" data-value="{{ date('Y/m/d') }}" clear="false"/>
        </x-row-horizontal>
        <x-row-horizontal label="Until Date">
            <x-pickerdate :dom="$dom" compid="euntildate" data-value="{{ date('Y/m/d') }}" clear="false"/>
        </x-row-horizontal>
        <x-row-horizontal label="Storage">
            <x-select :dom="$dom" compid="estorage" type="serverside" url="logbook/mapping/storage-temp/select" size="sm" :default="[$first_storage_name, $first_storage_name]"/>
        </x-row-horizontal>
    </x-form-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        report: "report/logbook/temperature/report",
        export: "report/logbook/temperature/export",
    },
    event: {
        excel: function () {
    ...
    func: {
        show: function () {
            ...
            var storage = fslctfstorage{{$dom}}.get().replace(" ", "%20") ;
