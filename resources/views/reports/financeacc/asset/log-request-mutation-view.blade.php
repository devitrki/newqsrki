                            ...
                            <div class="col-12">
                                <x-form-vertical>
                                    <x-row-vertical label="Plant">
                                        <x-select :dom="$dom" compid="fplant" type="serverside" url="master/plant/select?auth=true&ext=all&type=dc" size="sm" :default="[0, __('All') ]"/>
                                    </x-row-vertical>
                                    <x-row-vertical label="From Date">
                                        <x-pickerdate :dom="$dom" compid="ffromdate" data-value="{{ date('Y/m/d', strtotime('-1 month')) }}" clear="false"/>
                                    </x-row-vertical>
...
<x-modal :dom="$dom" compid="modalexport" title="Export Report">
    <x-form-horizontal>
        ...
        <x-row-horizontal label="Plant">
            <x-select :dom="$dom" compid="eplant" type="serverside" url="master/plant/select?auth=true&ext=all&type=dc" size="sm" :default="[0, __('All') ]"/>
        </x-row-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        report: "report/financeacc/log-request-mutation/report",
        export: "report/financeacc/log-request-mutation/export",
    },
    event: {
        excel: function () {
