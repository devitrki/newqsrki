...
@section('content')
    @if ($count > 0)
        ...
        <p class="text-center m-0">{{ __('Outstanding Asset Transfer Report') }}</p>
        </div>
        <div class="col-12">
                ...
                <tbody>
                    @foreach ($items as $i => $item)
                        <tr>
                            ...
                            <td data-label="Current Step">{{ $item->type }}</td>
                            ...
                            <td data-label="Est Transfer Date">{{ ($item->date_send_est != '' && $item->date_send_est != null) ? App\Library\Helper::DateConvertFormat($item->date_send_est, 'Y-m-d H:i:s', 'd-m-Y') : '-' }}</td>
                            <td data-label="Request Date">{{ ($item->date_request != '' && $item->date_request != null) ? App\Library\Helper::DateConvertFormatTz($item->date_request, 'Y-m-d H:i:s', 'UTC', 'd-m-Y H:i:s', $item->company_id) : '-' }}</td>
                            <td data-label="Approver 1 Date">{{ ($item->date_approve_first != '' && $item->date_approve_first != null) ? App\Library\Helper::DateConvertFormatTz($item->date_approve_first, 'Y-m-d H:i:s', 'UTC', 'd-m-Y H:i:s', $item->company_id) : '-' }}</td>
                            <td data-label="Confirm Validator Date">{{ ($item->date_confirmation_validator != '' && $item->date_confirmation_validator != null) ? App\Library\Helper::DateConvertFormatTz($item->date_confirmation_validator, 'Y-m-d H:i:s', 'UTC', 'd-m-Y H:i:s', $item->company_id) : '-' }}</td>
                            <td data-label="Approver 2 Date">{{ ($item->date_approve_second != '' && $item->date_approve_second != null) ? App\Library\Helper::DateConvertFormatTz($item->date_approve_second, 'Y-m-d H:i:s', 'UTC', 'd-m-Y H:i:s', $item->company_id) : '-' }}</td>
                            <td data-label="Confirm Sender Date">{{ ($item->date_confirmation_sender != '' && $item->date_confirmation_sender != null) ? App\Library\Helper::DateConvertFormatTz($item->date_confirmation_sender, 'Y-m-d H:i:s', 'UTC', 'd-m-Y H:i:s', $item->company_id) : '-' }}</td>
