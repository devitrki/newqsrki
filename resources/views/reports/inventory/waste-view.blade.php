                        ...
                        <x-dropdown-filter :dom="$dom" dtblecompid="tabledata">
                            <div class="col-12">
                                <x-form-vertical>
                                    ...
                                    <x-row-vertical label="Hide No Data">
                                        <fieldset>
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" class="custom-control-input" checked="true" id="hide{{$dom}}">
                                                <label class="custom-control-label" for="hide{{$dom}}"></label>
                                            </div>
                                        </fieldset>
                                    </x-row-vertical>
...
<x-modal :dom="$dom" compid="modalexport" title="Export Report">
    <x-form-horizontal>
        ...
        <x-row-horizontal label="Plant">
            <x-select :dom="$dom" compid="eplant" type="serverside" url="master/plant/select?auth=true&ext=all" size="sm" :default="[0, __('All')]"/>
        </x-row-horizontal>
        <x-row-horizontal label="Hide No Data">
            <fieldset>
                <div class="custom-control custom-checkbox">
                    <input type="checkbox" class="custom-control-input" checked="true" id="ehide{{$dom}}">
                    <label class="custom-control-label" for="ehide{{$dom}}"></label>
                </div>
            </fieldset>
        </x-row-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        report: "report/inventory/waste/report",
        export: "report/inventory/waste/export",
    },
    event: {
        excel: function () {
    ...
    func: {
        show: function () {
            ...
            var url ={{$dom}}.url.report + '?plant-id=' + $("#select2fplant{{$dom}}").val() + '&from-date=' + pickerdateffromdate{{$dom}}.get('select', 'yyyy/mm/dd') + '&until-date=' + pickerdatefuntildate{{$dom}}.get('select', 'yyyy/mm/dd') + '&hide=' + $('#hide{{$dom}}').is(':checked');
