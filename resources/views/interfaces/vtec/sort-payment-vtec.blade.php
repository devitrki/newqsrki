<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="interfaces/vtec/sort-payment-vtec/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "interfaces/vtec/sort-payment-vtec",
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
