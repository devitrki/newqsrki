<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="interfaces/gion/accurate/mapping-payment/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<x-modal :dom="$dom" compid="modalmanage" title="Item">
    <x-form-horizontal>
        <x-row-horizontal label="name pos">
            <input type="text" class="form-control form-control-sm" id="name_pos{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="bank coa">
            <input type="text" class="form-control form-control-sm" id="bank_coa{{$dom}}">
        </x-row-horizontal>
    </x-form-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "interfaces/gion/accurate/mapping-payment",
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
    ...
    func: {
        ...
        set: function () {
            ...
            $("#name_pos{{$dom}}").val(data.name_pos);
            $("#bank_coa{{$dom}}").val(data.bank_coa);
            showModal('modalmanage{{$dom}}');
        },
        getDataForm: function () {
            return {
                'name_pos': $("#name_pos{{$dom}}").val(),
                'bank_coa': $("#bank_coa{{$dom}}").val(),
                'id': {{$dom}}.data.id
            }
        },
        save: function () {
