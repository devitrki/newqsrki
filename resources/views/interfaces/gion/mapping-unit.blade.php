            ...
            @php
                $columns =
                    [[
                        ...
                        'label' => 'Unit Name Accurate',
                        'data' => 'unit_name_accurate',
                    ]];
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="interfaces/gion/accurate/mapping-unit/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Unit">
    <x-form-horizontal>
        <x-row-horizontal label="Unit Name Pos">
            <input type="text" class="form-control form-control-sm" id="unit_name_pos{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Unit Name Accurate">
            <input type="text" class="form-control form-control-sm" id="unit_name_accurate{{$dom}}">
        </x-row-horizontal>
    </x-form-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "interfaces/gion/accurate/mapping-unit",
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
    ...
    func: {
        reset: function () {
            {{$dom}}.data.id = 0;
            $("#unit_name_pos{{$dom}}").val('');
            $("#unit_name_accurate{{$dom}}").val('');
        },
        set: function () {
            ...
            $("#unit_name_pos{{$dom}}").val(data.unit_name_pos);
            $("#unit_name_accurate{{$dom}}").val(data.unit_name_accurate);
            showModal('modalmanage{{$dom}}');
        },
        getDataForm: function () {
            return {
                'unit_name_pos': $("#unit_name_pos{{$dom}}").val(),
                'unit_name_accurate': $("#unit_name_accurate{{$dom}}").val(),
                'id': {{$dom}}.data.id
            }
        },
        save: function () {
