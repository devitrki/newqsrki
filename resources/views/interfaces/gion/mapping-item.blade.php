<x-card-scroll>
    <div class="row m-0">
        <div class="col-12 p-0 fit-content-tabs">
            ...
            @endphp
            <x-datatable-serverside :dom="$dom" compid="tabledata" :tabmenu="$menu_id" :columns="$columns" url="interfaces/gion/accurate/mapping-item/dtble" :select="[true, 'single']" />
        </div>
    </div>
</x-card-scroll>
...
<!-- modal -->
<x-modal :dom="$dom" compid="modalmanage" title="Item">
    <x-form-horizontal>
        <x-row-horizontal label="Name">
            <input type="text" class="form-control form-control-sm" id="name{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Alias Code">
            <input type="text" class="form-control form-control-sm" id="alias_code{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Portion">
            <input type="text" class="form-control form-control-sm" id="portion{{$dom}}">
        </x-row-horizontal>
        <x-row-horizontal label="Accurate Code">
            <input type="text" class="form-control form-control-sm" id="accurate_code{{$dom}}">
        </x-row-horizontal>
    </x-form-horizontal>
...
{{$dom}} = {
   data: {
    ...
    url: {
        save: "interfaces/gion/accurate/mapping-item",
    },
    event: {
        create: function () {
            {{$dom}}.func.reset();
    ...
    func: {
        ...
        set: function () {
            ...
            $("#alias_code{{$dom}}").val(data.alias_code);
            $("#portion{{$dom}}").val(data.portion);
            $("#accurate_code{{$dom}}").val(data.accurate_code);
            showModal('modalmanage{{$dom}}');
        },
        getDataForm: function () {
            return {
                'name': $("#name{{$dom}}").val(),
                'alias_code': $("#alias_code{{$dom}}").val(),
                'portion': $("#portion{{$dom}}").val(),
                'accurate_code': $("#accurate_code{{$dom}}").val(),
                'id': {{$dom}}.data.id
            }
        },
        save: function () {
