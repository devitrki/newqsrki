...
class ConfigurationController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('inventory.configuration', $dataview)->render();
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($success) {
            DB::commit();
            Cache::forget('configuration_comp_' . 'inventory' . $userAuth->company_id_selected);
            $stat = 'success';
            $msg = Lang::get("message.save.success", ["data" => Lang::get("inventory configuration")]);
        } else {
            DB::rollBack();
            $stat = 'failed';
            $msg = Lang::get("message.save.failed", ["data" => Lang::get("inventory configuration")]);
        }
