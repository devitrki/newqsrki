...
class LbDlyInvCashier extends Model implements Auditable
{
    ...
    public static function getDataReport($plantId, $fromDate, $untilDate = null)
    {
        ...
        if( $untilDate ){
            ...
            for ($i=0; $i <= $diffDay; $i++) {
                ...
                $query = DB::table('lb_dly_inv_cashiers as dic')
                            ->leftJoin('lb_app_reviews as ar', 'ar.id', '=', 'dic.lb_app_review_id')
                            ->select('dic.product_name', 'dic.uom', 'dic.frekuensi', 'dic.stock_opening', 'dic.stock_in', 'dic.stock_out',
                            'dic.stock_closing', 'dic.lb_app_review_id', 'dic.note')
    ...
    {
        $report_data = [
            'title' => Lang::get('FORM DAILY INVENTORY (CASHIER)'),
            'data' => LbDlyInvCashier::getDataReport($param->plant, $param->from_date, $param->until_date)
        ];

        $path = 'reports/logbook/daily-inventory-cashier/pdf/';
        $filename = 'report-daily-inventory-cashier-';
        $random = strtolower(Helper::generateRandomStr(8));
        $typefile = '.pdf';
        $pdf = PDF::loadView('logbook.pdf.daily-inventory-cashier-pdf', $report_data)->setPaper('A4', 'portrait')->setOptions(['isHtml5ParserEnabled' => true, 'isRemoteEnabled' => true]);
        $report = [];
